...
export function RecurringJobCard({ jobId, initial }: {
  jobId: string
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="flex items-center gap-2"><Repeat className="h-4 w-4 text-gray-400" /> Recurring</CardTitle>
      ...
      {on && (
        <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div><span className="text-xs text-gray-400">Next occurrence</span><Input type="date" value={next} onChange={e => setNext(e.target.value)} /></div>
      ...
      <CardContent className="pt-0">
        ...
        <p className="text-xs text-gray-400 mt-2">A fresh copy of this job is created automatically each period (runs with the daily reminders job).</p>
      </CardContent>
    </Card>
  )
}
